...
curl -s -X POST http://localhost:3000/scan/code \
  ...
  -d "{\"code\": \"const password = 'admin123'; eval(userInput); const apiKey = 'sk-prod-abc123'; const secretKey = 'mysupersecretkey1234567890abcdef'; const query = 'SELECT * FROM users WHERE id = ' + userId; const hash = require('crypto').createHash('md5').update(data).digest('hex'); const weakHash = require('crypto').createHash('sha1').update(data).digest('hex'); exec(userCommand); const token = Math.random().toString(36); console.log('password:', password);\"}" \
